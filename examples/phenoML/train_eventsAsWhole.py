...
class AE_3D_200_LeakyReLU(nn.Module):
    ...
    def __init__(self, feature_no=4):
        ...
        self.latent = nn.Linear(20, 15)
        self.dec_l1 = nn.Linear(15, 20)
...
def custom_normalise(df):
    ...
    for i in range(1, int(len(df.columns)/4)+1):
        no = str(i)
        df['E'+no] = df['E'+no] / 1000.0
        df['pt'+no] = df['pt'+no] / 1000.0

        df['eta'+no] = df['eta'+no] / 5 
        df['phi'+no] = df['phi'+no] / 3

        df['E'+no] = np.log10(df['E'+no].replace(0, np.nan))
        df['E'+no] = df['E'+no].replace(np.nan, 0)
        df['pt'+no] = np.log10(df['pt'+no].replace(0, np.nan))
        df['pt'+no] = df['pt'+no].replace(np.nan, 0)
    return df
...
# Arguments:
#     data: input numpy array with values of the 4-vector in initial order
#     col_names: indexable object with names of the 4 features in order same as data
def plot_training(data, col_names):
    for col in np.arange(8):
        plt.figure()
        plt.hist(data[:, col], bins=200, alpha=1, histtype='step')
...
def main():
    ...
    # Model set-up
    model = AE_3D_200_LeakyReLU(feature_no=len(data.columns))
    model.to('cpu')
